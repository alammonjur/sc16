...
At the time the {\it insert} is being procesed, the {\it min-delete} comes and replaces the root with the element at node $H[11]$, creating hole at node $H[11]$, as shown at Figure \ref{hole-after}. 
...
So, any consecutive {\it insert-delete} will lead to hole creation with the root replacement mechanism for other heaps, regardless whether the {\it insert} comes first or second.

\subsubsection{How Holes are Minimized} 
When a hole is created in our proposed heap, its location is saved into a register. 
