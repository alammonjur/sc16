...
The replacement techniques we use at the previous section may solve this hole problem here. 
...
% The insert algorithm in Bhagwan and Lin's pipelined heap \ref{hw2} always fill empty nodes starting from the left going down. For each node, it considers whether an empty node exists in the left child path. If not, it moves to the right child path. In a sense, this 