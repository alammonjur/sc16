...
In general, any insert-delete combination will create a hole if $(t_j - t_i) < \beta\tau$, where $\beta$ is the depth of the heap and $\tau$ is the cycle time per level. $\beta - 1$ holes can be created in a $\beta$-level heap at most in an {\it insert} followed by consecutive {\it min-deletes}. In the worst case scenario, $\beta_max - 1$ holes during the propagation of an {\it insert}, where $\beta_max$ is the maximum depth of the heap. 

\subsection {Handling Holes}
...
Since $\beta_max - 1$ holes can created at most during the propagation of an {\it insert}, $\beta - 1$ registers are required.
