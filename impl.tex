...
\begin{figure}[!ht]
  ...
  \includegraphics[width=7cm]{fig/fillhole.png}
      \caption{Hole Minimization: there was a hole between 53 and 100. This gets filled up by 41 after inserting 3 into the heap}
    \label{fillhole}
\end{figure}
When whole is created, its location is saved into a register. The inserted node goes to that location instead of the last available location. For example, as shown at Fig \ref{hole-latches}, the hole index 10 is stored at hole register. The last available insert node location is 12 in normal implementation. Now, let us insert 3 into the heap. We consider the hole index 10 for constructing the insert path.  This hole is filled by the other element (41 here) during the instert operation as shown at Fig \ref{fillhole}. According to our algorithm, the insert path is $H[0] \rightarrow  H[1] \rightarrow H[4] \rightarrow H[10]$. The naive implementations consider the index 12 for the insert path; as a result their path would be  $H[0] \rightarrow  H[2] \rightarrow H[5] \rightarrow H[12]$, which does not gaurantee the hole minimization.

\subsubsection{Root Replacement by Last Element}
Some authors replace the root with the last element at delete operation \cite{hw2,hw3}. It seems that hole will not be created here. But this is not the case. Before the completion of pipeline propagation of delete operation, insert operation will lead to hole. For example, the initial data is as mentioned at Fig \ref{fig7}. Now perform two consecutive operations, insert and delete, no matter in which order. The resultant structure is shown at Fig \ref{hole-latches}. The hole location can be changed based on the data it already has. The hole will not be the outcome for only one situation: if the both insert and delete path end at the last node. They, indeed never touch these issues at all.

It seems that the replacement techniques we used at the previous section may solve this hole problem here. Because, there are no new data to be deleted, only root is replaced by the newly inserted element. But the next delete will be stalled till the completion of the operation of the full pipelined path. Here, the replacement can be apply only when the last location gets its correct element.
